...
def evaluate_audio(language, dataset, transcribe_fn):
    ...
    with open(os.path.join("evaluation-results", args.output_file), 'w') as file:
        file.write(f"[INFO] Evaluated {args.model_type} Speech-to-Text with {args.dataset_name} with language {args.language}/{args.config}\n\n")
...
if __name__ == "__main__":
    ...
    # parse input
    parser = argparse.ArgumentParser(description="Whisper ASR Evaluation - Industry Speech to Text")

    parser.add_argument("--model_type", type=str, required=True, default="google", help="STT model to evaluate on. (google, watson) - (default: google)")
    ...
    parser.add_argument("--language", type=str, default="ko-KR", help="Language code for model API (default: 'vi-VN' for Vietnamese for Google STT)")
    parser.add_argument("--split", type=str, default="test", help="The dataset split to evaluate on (default: 'test').")
    parser.add_argument("--output_file", type=str, default="whisper_eval_stt", help="Name of the file to save the evaluation results.")
    ...
    if args.model_type not in ["google", "watson"]:
        raise ValueError("Model not supported. Please choose (azure, google or watson)")
    ...
    evaluate_audio(language=args.language, dataset=dataset, transcribe_fn=model_map[args.model_type])
